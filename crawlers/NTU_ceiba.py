...
class CeibaCrawler(BaseCrawler):
    ...
    def _get_announcements_from_class_id(self, class_id):
        test_req = self.s.get(self.base_url + f"modules/bulletin/bulletin.php?default_lang=chinese&csn={class_id}")
        ...
        it = 0
        while True:
            url = self.base_url + "modules/bulletin/bulletin.php?startrec=" + str(it)
            req = self.s.get(url)
            req.encoding = "UTF-8"
            table = bs4(req.text, "html.parser").find("table")
            ...
            for r in rows:
                ...
                url = self.base_url + f"modules/bulletin/bulletin_popup.php?current_lang=chinese&sn={sn}&csn={class_id}"
